...
labelMat <- read_csv(file.path('results', '001_arregloOrigen.csv'))  %>%
...
# 
for (i in 1:81) {
  subdir <- glue('{aux_dir}/E{i}/modeloFinal')
...
pca_fct_ls1 %>% write_csv(file.path('results', '03_datosEstandarizados.csv'))
...
write_csv(PCdf, file.path('results', '02_PCA.csv'))
...
g1a <- plotTukeyComparisons(aov_1,'graph')$Plot + 
  labs(title = bquote('Variable distancias'~(italic(d)[i]~-~bar(italic(d))) ))
g2a <- plotTukeyComparisons(aov_2,'graph')$Plot +
...
use_python(file.path('C:', 'Users' ,'Daniel', 'AppData', 
                     'Local', 'Programs', 'Python', 'Python38'), T)
...
gComparativoParLL <-
  ...
  mutate(parameter = factor(parameter, levels = append(level_par, 'LL'))) %>%
  ggplot(aes(x = value, col = outlier, lty = outlier)) + 
  geom_density() + 
  ...
  scale_linetype_manual(values = c('solid', 'dashed'), breaks = c(F, T), name = 'LL < 400')
