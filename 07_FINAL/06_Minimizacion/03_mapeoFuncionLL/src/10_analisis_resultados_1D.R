...
df_beta_Cl_logtCLCRMLMIN <- extractor('beta_Cl_logtCLCRMLMIN')
...
df_beta_Cl_logtCLCRMLMIN1 <- df_beta_Cl_logtCLCRMLMIN %>% xval.func(beta_Cl_logtCLCRMLMIN, LL1)
...
G_ll_2 <- ggplot(df_beta_Cl_logtCLCRMLMIN, aes(x = beta_Cl_logtCLCRMLMIN, y = LL1)) +
  geom_point(data = df_beta_Cl_logtCLCRMLMIN1$Minimo, col = 'red3', shape = 8) +
  geom_point(data = df_beta_Cl_logtCLCRMLMIN1$Punto_Corte, col = 'green1') +
  geom_vline(data = popParam, aes(xintercept = beta_Cl_logtCLCRMLMIN), lty='dashed', col='blue4') + 
  xlab(expression(beta~'Cl-logtClCr')) + aux_plot

G_ll_3 <- ggplot(df_corr_V2_V1, aes(x = corr_V2_V1, y = LL1)) +
...
G_ll <- ((G_ll_1 + G_ll_2) / (G_ll_3 + plot_spacer())) + 
...
x_beta_Cl_logtCLCRMLMIN <- extractor('beta_Cl_logtCLCRMLMIN', mode='interval')
x_corr_V2_V1            <- extractor('corr_V2_V1', mode='interval')

# Cálculo de valores mínimos y adición de 3.84
x_Clpop1                 <- x_Clpop %>% xval.func(Cl_pop, LL_mn)
x_beta_Cl_logtCLCRMLMIN1 <- x_beta_Cl_logtCLCRMLMIN %>% xval.func(beta_Cl_logtCLCRMLMIN, LL_mn)
...
x_ll_2 <- ggplot(x_beta_Cl_logtCLCRMLMIN, aes(x = beta_Cl_logtCLCRMLMIN, y = LL_mn)) +
  geom_ribbon(aes(ymin=LL_li, ymax=LL_ls), fill=alpha('blue', 0.1)) +
  geom_point(data = x_beta_Cl_logtCLCRMLMIN1$Minimo, col = 'red3', shape = 8) +
  geom_point(data = x_beta_Cl_logtCLCRMLMIN1$Punto_Corte, col = 'green1') +
  geom_vline(data = popParam, aes(xintercept = beta_Cl_logtCLCRMLMIN), lty='dashed', col='blue4') + 
  xlab(expression(beta~'Cl-logtClCr')) + aux_plot

x_ll_3 <- ggplot(x_corr_V2_V1, aes(x = corr_V2_V1, y = LL_mn)) +
  geom_ribbon(aes(ymin=LL_li, ymax=LL_ls), fill=alpha('blue', 0.1)) +
...
x_ll <- ((x_ll_1 + x_ll_2) / (x_ll_3 + plot_spacer())) + 
  plot_annotation(tag_levels = 'A')
...
modelLs <- list()
modelLs[['Cl']]                 = x_Clpop1
modelLs[['beta_Cl_logtCLCRMLMIN']] = x_beta_Cl_logtCLCRMLMIN1
