...
# Definición Carpetas
homedir <- file.path('modeloFinal1')
...
G_PRED_OBS_PRED <- 
  GOF_PRED(y_gof, popPred, y, YTYPE, 
           ...
           legend = c(0.16, 0.82)) +
  predictivePerformaceLabel(y_gof, 'popPred', 'y', 0.94, round=3, boot = F)
...
G_PRED_OBS_IPRED <- 
  ...
  predictivePerformaceLabel(y_gof, 'indivPredMean', 'y', 0.7, round=3, boot = F)
