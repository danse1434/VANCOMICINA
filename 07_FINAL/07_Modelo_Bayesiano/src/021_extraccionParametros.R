...
predictNames <- as.matrix(fit) %>% 
  colnames() %>% 
  keep(~ str_detect(.x, '(cHat|cHatPred|cObsPred|cObsCond)(1|2)\\[\\d+\\]'))
...
findParameters <- function(x, dist='log-normal') {
  if (dist == 'log-normal'|dist == 'normal') {
...
createDistribution <- function(x, y, dist = 'log-normal') {
  if (dist == 'log-normal') {
    ...
    x %>% 
      mutate(pred = dnorm(values, y$estimate[['mean']], y$estimate[['sd']]) )
  } else {
...
ggsave(paste0(modelName, '_distBayesParam_1.pdf'), g, 'pdf', 'figures', 1, 8,6)
...
distDF2['mean'] <- distDF2$param$mean 
distDF2['sd'] <- distDF2$param$sd
...
distGT <- distDF2 %>% 
  ...
  tab_spanner(columns=4:5, label='Normal') %>%
  tab_spanner(columns=6:7, label='Gamma') %>%
  tab_header(
    title = md('**Parámetros de distribuciones estimadas Stan**'), 
    subtitle = 'Modelo Final estimado con Stan') %>% 
  tab_options(table.font.size = '12px')

gtsave(
  distGT,
  paste0(modelName, '_distBayesParam.html'),
...
# Almacenamiento de distribuciones empíricas y modelos de distribución
ggsave(paste0(modelName, '_distBayesParam_2.pdf'), distrCompar, 'pdf', 'figures', 1, 8,6)
