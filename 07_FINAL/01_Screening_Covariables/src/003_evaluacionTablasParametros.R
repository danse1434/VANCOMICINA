...
data1 <- read_csv(file.path('data', 'processed', 
                            '001_parametrosIndividuales.csv'), 
                  col_types = cols())
...
data1 <- data1 %>% 
  filter(EVID == 1) %>% 
  select(all_of(covariates), matches('^(Cl|Q|V1|V2)\\_SAEM'))

#-------------------------------------------------------------------------------#
# 2. Tablas con resultados de regresión por eta ------------------------------
#-------------------------------------------------------------------------------#
vectorPars <- c('Cl', 'Q', 'V1', 'V2')
vectorEtas <- c('Cl_SAEM', 'Q_SAEM', 'V1_SAEM', 'V2_SAEM')
...
gt_linear <- df  %>% 
  ...
  tab_header(title = md('**Resultados de evaluación modelos lineales**'), 
             subtitle = 'Variable dependiente: parámetro') %>% 
...
gtsave(gt_linear, '008_resumen_modelosLineales_parametro.html', 
...
gtsave(gt_spearman, '009_resumen_Spearman_parametro.html', 
       file.path('figures') %>% normalizePath())
...
gtsave(gt_GAM, '010_resumen_GAM_parametro.html', 
       file.path('figures') %>% normalizePath())
