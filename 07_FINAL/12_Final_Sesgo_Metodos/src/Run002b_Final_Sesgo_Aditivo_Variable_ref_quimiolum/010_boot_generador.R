...
project.file <- file.path(auxdir, 'FINAL.mlxtran')
paramet.file <- file.path(auxdir, 'FINAL')
...
for (i in 1:1000) {
  file.copy(file.path(glue('FINAL.mlxproperties')),
            file.path(bootdir, glue('B{i}/')))
}

#-------------------------------------------------------------------------------#
# Apertura del archivo de control
fileName <- file.path('FINAL.mlxtran')
...
for (i in 1:1000) {
  ...
  write_lines(B,
              file.path(bootdir, glue('B{i}/FINAL.mlxtran')), sep = '\n')
}
