...
# 1. Introducción -----------------------------------------------------
#-------------------------------------------------------------------------------#
# IMPORTANTE: CORRER CON VERSION R 3.6.3
# Carga de paquetes
...
#-------------------------------------------------------------------------------#
auxdir <- file.path('.', 'Run002b_Final_Sesgo_Aditivo_Variable_ref_quimiolum')
project.file <- file.path('Run002b_Final_Sesgo_Aditivo_Variable_ref_quimiolum.mlxtran')
simulx.file <- file.path("model", 'Run002b_Final_Sesgo_Aditivo_Variable_ref_quimiolum_smlx.txt')
paramet.file <- file.path('Run002b_Final_Sesgo_Aditivo_Variable_ref_quimiolum')
...
sim.param['omega_Cl'] = 0
sim.param['omega_V1'] = 0
sim.param['omega_Q'] = 0
sim.param['omega_V2'] = 0
sim.param['bias_sd'] = 0
sim.param['a1'] = 0 
sim.param['a2'] = 0
...
data_pred <- simulx(
  model = simulx.file,
  parameter = list(sim.param, covLev),
  treatment = data_TAD$treatment,
  output = list(out1, out2),
  settings = list(load.design = TRUE, digits = 3)
)
...
dfr_percs_1 <- dfr_percs %>% 
  .[, .(
    TIME = mean(time),
    ME   = quantile(x = pc_y, probs = 0.50, na.rm = TRUE),
    LI   = quantile(x = pc_y, probs = 0.10, na.rm = TRUE),
    LS   = quantile(x = pc_y, probs = 0.90, na.rm = TRUE)
  ), by = .(YTYPE, design, gr)]
