...
## Fecha de modificación: 20-ago-2022
##  
...
# Apertura de fuente
source(file.path('src', '002_funciones.R'), encoding = 'UTF-8')
...
G_PRED_OBS_PRED <- 
  GOF_PRED(x = popPred, y = y1, 
           ...
           colourp = 'blue1', xlab = 'PRED (mg/L)', ylab = 'OBS (mg/L)')
...
G_PRED_OBS_IPRED <- 
  GOF_PRED(x = indivPredMean, y = y1, 
           ...
           colourp = 'red1', xlab = 'IPRED (mg/L)', ylab = 'OBS (mg/L)')
...
popeta1 <- popeta %>% 
  filter(str_detect(parameter, "(?:omega\\_)|(?:_sd)")) %>% 
