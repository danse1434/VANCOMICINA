...
# Definición Carpetas
homedir <- file.path('Run002b_Final_Sesgo_Aditivo_Variable_ref_quimiolum')
...
G_PRED_OBS_PREDLOG <- 
  GOF_PRED(y_gof, popPred, y, YTYPE, 
           xlab = 'Population predicted \n concentrations (mg/L)', 
           ylab = 'Observed concentrations \n (mg/L)', xlim = c(2, 100), ylim = c(2, 100)) + 
...
G_RES_C_PWRES <- RES_PRE(
  y_residuals, prediction_pwRes, pwRes, YTYPE, ID, 7, 'Population predicted \n concentrations (mg/L)', 'PWRES',
  ylim = c(-5, 5), colourp = c('#8721B8', '#B82D18'), legend = c(0.5, 0.08))
G_RES_C_IWRES <- RES_PRE(
  y_residuals, prediction_iwRes_mean, iwRes_mean, YTYPE, ID, 7, 'Individual predicted \n concentrations (mg/L)', 'IWRES',
  ylim = c(-5, 5), colourp = c('#8721B8', '#B82D18'))
G_RES_C_NPDE  <- RES_PRE(
  y_residuals, prediction_npde , npde, YTYPE, ID, 7, 'Population predicted \n concentrations (mg/L)', 'NPDE',
  ylim = c(-5, 5), colourp = c('#8721B8', '#B82D18'))
...
G_CP_TAD <-  yfits %>% 
  arrange(ID, time) %>%
  mutate(YTYPE = if_else(YTYPE == 1, "Micro.", "Quimiol.")) %>% 
  ggplot(aes(x = time, color = YTYPE, group = YTYPE)) +
  geom_ribbon(yobs, 
...
popeta1 <- popeta %>% 
  ...
  .[grep('(?:omega\\_)|(?:_sd)', popeta$parameter), c('parameter', 'value')] %>% 
  pivot_wider(names_from = parameter, values_from = value) %>% 
  rename(omega_bias =bias_sd)
