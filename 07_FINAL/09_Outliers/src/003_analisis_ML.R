...
source(file.path('src', '010_fun_analisis.R'), encoding = 'UTF-8')
...
for (i in 1:14) {
  data_vector[[i]] = read_csv(file.path('models', glue('evalJack{i}'), 'populationParameters.txt'),
                              col_types = cols())
}
...
data_gt <- data_df %>%
  gt() %>%
  fmt_number(matches('pop$|^beta'), decimals = 2) %>%
  fmt_number(matches('omega|corr|a'), decimals = 3) %>%
  tab_header(
    title = 'Modelo Final Vancomicina - Análisis de Influencia', 
...
rec <- recipe(Sujeto ~ ., data = data_df)
nor <- rec %>% step_normalize(all_predictors()) 
pre <- nor %>% prep(data_df)
...
G1 <- bake(pre, data_df) %>%
  ...
  mutate(
    parameter = factor(parameter, levels = par_lev), 
...
# G1
ggsave("1_valores_estandarizados.pdf", G1, 'pdf', 'figures', 1, 5*1.5, 4*1.5)
...
# Los cuatro primeros componentes explican 37.93%, 22.33%, 13.273%, y 12.496% de la 
...
# Para PC2 se identifica al individuo 1 como posible individuo atípico, sólo 
# con el método robusto
...
gPCA_LOF <-
  ...
  geom_point(aes(color = 'Observaciones', shape='Observaciones'), size=sh) +
  geom_point(
...
plot3D_PCA <- function() {
  ...
  plot3D::scatter3D(
    ...
    xlim = c(-7.0, 7.0), ylim = c(-7.0, 7.0), zlim = c(-7.0, 7.0)
  )
...
#-------------------------------------------------------------------------------#
# 5. Gráfico 3D con componentes principales ------------
#-------------------------------------------------------------------------------#
require(plotly)
...
fig <-
  fig %>% layout(
    scene = list(
      xaxis = list(title = 'PC1 (34.6%)'),
      yaxis = list(title = 'PC2 (22.6%)'),
      zaxis = list(title = 'PC3 (14.5%)')
    ),
    title = paste0(
      'Evaluación de influencia de individuos Modelo Final VAN <br>',
      '<sup>Análisis de outlier por Jacknife Del-1</sup>'
    )
  )

htmlwidgets::saveWidget(
  fig,
  file.path('figures', '5_grafico3D_PCA.html') %>% normalizePath(),
  selfcontained = T
)
