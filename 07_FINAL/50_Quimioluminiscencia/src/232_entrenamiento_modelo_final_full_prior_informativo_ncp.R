...
source(file.path('src', '226_preparacion_modelo_final_full_prior_informativo.R'), encoding = 'UTF8')
source(file.path('src', '280_fun_funcion2Cptm.R'), encoding = 'UTF8')

modelName <- '230_modelo_final_full_prior_informativo_ncp'
run_file <- "run200"
...
df_out <- as.data.frame(fit, pars = parameters) %>% 
  tibble() %>% 
  mutate(across(contains("omega"), ~ sqrt(exp(.x) - 1))) %>% 
...
df_out["Parameter"] <- c("CL (L/h)", "Q (L/h)", "V1 (L)", "V2 (L)",
                         "theta CL ~ CLCR", 
                         "Omega CL (CV%)", "Omega Q (CV%)", 
                         "Omega V1 (CV%)", "Omega V2 (CV%)",
                         "rho (CL,Q)", "rho (CL,V1)", "rho (CL,V2)",
                         "rho (Q,V1)", "rho (Q,V2)", "rho (V1,V2)",
                         "b (%)")
...
gt_out <- gt(df_out) %>% 
  ...
  tab_header(title = "Modelo bayesiano - Run 230", 
