...
model{
    ...
    omega_Q ~ cauchy(mu_Omega_Q, sd_Omega_Q);
    
    b ~ cauchy(mu_b, sd_b);
    
    // Variabilidad Inter-individual
    logTheta ~ multi_normal(log( segment(thetaHat, 1, 2) ), Omega);
    logThetaQ ~ normal(log(thetaHat[3]), omega_Q);
