...
parameters{
  ...
  cholesky_factor_corr[nRandom] rho;
  real<lower = 0> b;
  
  matrix[nRandom, nSubjects] alpha;
}

transformed parameters{
  vector<lower = 0>[nFixed] thetaHat;
  matrix[nFixed, nSubjects] logTheta;
  ...
  // Efectos Aleatorios
  logTheta = rep_matrix(log(thetaHat), nSubjects) + diag_pre_multiply(omega, rho * alpha);
  
  // Transformaciones
  CL = to_array_1d(exp(to_vector(logTheta[1,]) + (to_vector(logtCLCR) * beta_Cl_logtCLCR)));
  V1 = to_array_1d(exp(logTheta[2,]));
  Q  = to_array_1d(exp(logTheta[3,]));
  V2 = to_array_1d(exp(logTheta[4,]));
...
model{
    CLHat ~ normal(mu_ClHat, sd_ClHat);
    V1Hat ~ normal(mu_V1Hat, sd_V1Hat);
    QHat  ~ normal(mu_QHat, sd_QHat);
    V2Hat ~ normal(mu_V2Hat, sd_V2Hat);
    
    beta_Cl_logtCLCR ~ normal(mu_beta_Cl_logtCLCR, sd_beta_Cl_logtCLCR);
    rho ~ lkj_corr_cholesky(rho_prior); 
    ...
    // Implica logTheta ~ multi_normal(log(thetaHat), diag_matrix(omega) * L * L' * diag_matrix(omega));
...
generated quantities{
  matrix[nFixed, nSubjects] logthetaPred;
  ...
  matrix[nRandom, nSubjects] alphaPred;
  
  for(i in 1:nRandom){
    for(j in 1:nSubjects){
      alphaPred[i,j] = normal_rng(0, 1);
    }
  }
  logthetaPred = rep_matrix(log(thetaHat), nSubjects) + diag_pre_multiply(omega, rho * alphaPred);
  
  CLPred = to_array_1d(exp(to_vector(logthetaPred[1,]) + (to_vector(logtCLCR) * beta_Cl_logtCLCR)));
  QPred  = to_array_1d(exp(logthetaPred[2,]));
  V1Pred = to_array_1d(exp(logthetaPred[3,]));
  V2Pred = to_array_1d(exp(logthetaPred[4,]));
