...
parameters{
  ...
  vector[nFixed-1] logTheta[nSubjects];
}
...
transformed parameters{
  ...
  V1 = exp(logTheta[,2]);
  Q  = exp(logTheta[,3]);
  V2 = rep_array(thetaHat[4], nSubjects);
...
model{
    ...
    rho ~ lkj_corr(rho_prior); 
    omega ~ cauchy(mu_Omega, sd_Omega);
    
    b ~ cauchy(mu_b, sd_b);
    
    // Variabilidad Inter-individual
    logTheta ~ multi_normal(log( segment(thetaHat, 1, 3) ), Omega);
