...
project.file <- file.path('run200.mlxtran')
simulx.file <- file.path('run200_simulx.txt')
paramet.file <- file.path('run200')
...
dfr_percs <- rbind(dfr_percs_2[, YTYPE := 2])
...
ggsave(file.path(auxdir, 'figures', 'pcVPC_percentil.pdf'), g_percs, 'pdf',
       width = 6, height = 0.65 * 6)

# Almacenar RDS
saveRDS(g_percs, file.path(auxdir, 'figures', 'RDS', 'pcVPC_percentil.rds'))
