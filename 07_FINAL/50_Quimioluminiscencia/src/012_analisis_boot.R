...
#-------------------------------------------------------------------------------#
rundir  <- file.path('run200')
bootdir <- file.path(rundir, 'boot_results')
fig_path <- file.path(rundir, "figures")
...
vectorParametros <- c('Cl_pop', 'beta_Cl_logtCLCR',
                      'V1_pop', 'V2_pop', 'Q_pop',
                      'omega_V1', 'omega_Cl', 'omega_Q', 'corr_V1_Cl',
                      'b', 'k10', 'k12', 'k21', 'alpha', 'beta',
                      't_alpha', 't_beta', 'A', 'B')

popParameters <- read_csv(file.path(rundir, 'populationParameters.txt'))
...
for (i in 1:1000) {
  ...
  param_list[[i]] = read_csv(
    str_glue('{bootdir}/B{id}/run200/populationParameters.txt'),
...
ggsave(str_glue('{fig_path}/1000_resultados_bootstrap.pdf'), G1, 'pdf',
       width = 9, height = 8)
...
# Almacenamiento del gráfico
ggsave(str_glue('{fig_path}/1001_originales_bootstrap.pdf'), G2, 'pdf',
       width = 8, height = 6)
...
# Almacenamiento del gráfico
ggsave(str_glue('{fig_path}/1002_bootstrap_dots.pdf'), G3, 'pdf',
       width = 6, height = 4)
...
param_table_gt <- param_table %>%
  ...
  gt::gt() %>%
  gt::fmt_number(columns = c('mn', 'sd', 'rsd', 'li', 'ls'),
                 decimals = 2) %>%
  gt::cols_merge_range(col_begin = c(li), col_end = c(ls)) %>%
  gt::cols_label(
    parameter = 'Parámetro',
    mn = 'Media',
    sd = 'SE',
  ...
  gt::tab_header(gt::md('**Resultados bootstrap no parámétrico**'),
                 gt::md('**Modelo final para VAN método quimiluminiscencia - error proporcional**')) %>% 
  gt::tab_options(table.font.size = "smaller",
                  data_row.padding = gt::px(3)) %>% 
  gt::tab_footnote('Obtenido mediante bootstrap no paramétrico', 
                   gt::cells_column_labels(columns = c(li))) %>% 
  gt::tab_footnote('Obtenido mediante matriz de info. de Fisher (FIM)', 
                   gt::cells_column_labels(columns = c(sd, rsd)))


# Almacenamiento de tabla de datos de HTML
gt::gtsave(param_table_gt, '1003_resultados_bootstrap.html', 
           str_glue('{fig_path}/') %>% normalizePath())
...
for (i in 1:1000) {
  convergence_list[[i]] =
    read_csv(
      str_glue(
        '{bootdir}/B{sprintf("%03d", i)}/run200/ChartsData/Saem/CvParam.txt'
      ),
      col_types = cols()
    )
}
...
ggsave(str_glue('{fig_path}/1006_filtro_convergencia.pdf'), G_conv_filt, 'pdf', 
         width = 7, height = 5)
...
# Almacenamiento del gráfico
ggsave(str_glue('{fig_path}/1007_resultados_bootstrap_limpio.pdf'), G1b, 'pdf',
       width = 8, height = 6)
...
# Almacenamiento del gráfico
ggsave(str_glue('{fig_path}/1008_bootstrap_dots_limpia.pdf'), G3b, 'pdf',
       width = 6, height = 4)
...
param_df3_gt <- param_df3 %>%
  ...
  gt::gt() %>%
  gt::fmt_number(columns = c(value,se_sa,rse_sa,
                  median,classic.LI,classic.LS,percent.LI,percent.LS,
                  normal.LI,normal.LS,pivote.LI,pivote.LS,BCa.LI,
                  BCa.LS),
                 decimals = 2) %>% 
  gt::cols_merge_range(c(classic.LI), c(classic.LS), sep = I(", ")) %>% 
  gt::cols_merge_range(c(percent.LI), c(percent.LS), sep = I(", ")) %>% 
  gt::cols_merge_range(c(normal.LI), c(normal.LS), sep = I(", ")) %>% 
  gt::cols_merge_range(c(pivote.LI), c(pivote.LS), sep = I(", ")) %>% 
  gt::cols_merge_range(c(BCa.LI), c(BCa.LS), sep = I(", ")) %>% 
  gt::tab_spanner(label = str_glue("Resultados Bootstrap (n = {unique(param_df3$n)})"), 
                  c(median, classic.LI,classic.LS,percent.LI,percent.LS,
                       normal.LI,normal.LS,pivote.LI,pivote.LS,BCa.LI,
                       BCa.LS)) %>% 
  gt::cols_label(
    parameter = 'Parámetro',
  ...
  gt::tab_header(gt::md('**Resultados bootstrap no parámétrico**'),
                 gt::md('**Modelo final VAN por método de quimioluminiscencia - error proporcional**')) %>% 
  gt::tab_options(table.font.size = "smaller",
                  data_row.padding = gt::px(3)) %>% 
  gt::tab_footnote('Obtenido mediante bootstrap no paramétrico', 
                   gt::cells_column_spanners(
                     spanners = str_glue("Resultados Bootstrap (n = {unique(param_df3$n)})"))) %>% 
  gt::tab_footnote('Obtenido mediante matriz de info. de Fisher (FIM)', 
                   gt::cells_column_labels(columns = c(se_sa, rse_sa)))


# Almacenamiento de tabla de datos de HTML
gt::gtsave(param_df3_gt, '1009_IC_bootstrap.html', 
           str_glue('{getwd()}/{fig_path}/'))
