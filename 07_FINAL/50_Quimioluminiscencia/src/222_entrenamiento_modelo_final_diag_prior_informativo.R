...
source(file.path('src', '221_preparacion_modelo_final_diag_prior_informativo.R'), encoding = 'UTF8')
source(file.path('src', '280_fun_funcion2Cptm.R'), encoding = 'UTF8')

modelName <- '220_modelo_final_diag_prior_informativo'
run_file <- "run200"
...
gt_out <- gt(df_out) %>% 
  ...
  tab_header(title = "Modelo bayesiano - Run 220", 
             subtitle = "Matriz IIV diagonal, error proporcional")
