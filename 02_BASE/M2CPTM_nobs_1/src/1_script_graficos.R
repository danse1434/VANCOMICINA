...
source("M2CPTM_nobs_1/src/2_funciones.R", encoding = 'UTF-8')
...
auxdir <- file.path('M2CPTM_nobs_1', 'm2cptm_nobs1')
...
ggsave(file.path(auxdir, 'figures/G_GOF.pdf'), G1, 
...
ggsave(file.path(auxdir, 'figures/G_RES.pdf'), G2, 'pdf', 
       width = 8, height = 8, units = 'in')

G2b <- G_RES_T_PWRES + G_RES_T_IWRES + G_RES_T_NPDE  +
  G_RES_C_PWRES + G_RES_C_IWRES + G_RES_C_NPDE +
  plot_annotation(tag_levels = 'A') + 
  plot_layout(ncol = 3)

ggsave(file.path(auxdir, 'figures/G_RES_b.pdf'), G2b,  
...
ggsave(file.path(auxdir, 'figures/G_CP_TAD.pdf'), G_CP_TAD, 'pdf', 
       width = 5, height = 6)
...
gt_shrinkage %>% 
  gt::gtsave('Shrink.html', file.path(getwd(), auxdir, 'figures'))
