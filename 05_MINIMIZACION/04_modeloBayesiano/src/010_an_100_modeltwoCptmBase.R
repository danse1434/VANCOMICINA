source('./src/000_prepro_100_modeltwoCptmBase.R', encoding = 'UTF8')
source('./src/051_fun_funcion2Cptm.R', encoding = 'UTF8')

modelName <- '100_modeltwoCptmBase'
...
if(!file.exists(file.path('models', paste0(modelName, "Fit.Rsave")))){
  ...
  fit <- stan(file = file.path('src', paste0('100_modeltwoCptmBase', ".stan")),
              data = stan_d,
              iter = nIter,
              warmup = nBurnin,
              thin = nThin, 
...
parameters <- as.matrix(fit) %>% 
  colnames() %>% 
  keep(~ str_detect(.x, 'CLHat|QHat|V1Hat|V2Hat|omega|rho|sigma')) %>% 
