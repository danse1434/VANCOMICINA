source('./src/003_prepro_103_modeltwoCptmDiagPropCov.R', encoding = 'UTF8')
source('./src/051_fun_funcion2Cptm.R', encoding = 'UTF8')

modelName <- '103_modeltwoCptmDiagPropCov'
...
parameters <- as.matrix(fit) %>% 
  colnames() %>% 
  keep(~ str_detect(.x, 'CLHat|QHat|V1Hat|V2Hat|omega|rho|^b\\d$')) %>% 
...
gPRED1 <- predictions1 %>% 
  ...
  labs(title='Quimioluminiscencia')
...
gPRED2 <- predictions1 %>% 
  ...
  theme_bw()+
  xlab('Tiempo (hr)') + ylab(bquote(C[PRED]~(mg/L))) +
  labs(title='Microbiológico')


require(patchwork)

ggsave(file.path('figures', paste0(modelName, 'CPRED.pdf')),
       plot = gPRED1 + gPRED2 + plot_annotation(title = 'Modelo Combinado'), 
       device = 'pdf', 
       width = 12, height = 7)
