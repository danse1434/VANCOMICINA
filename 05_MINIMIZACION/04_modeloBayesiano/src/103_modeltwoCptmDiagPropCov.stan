...
model{
    ...
    for(i in 1:nObs){
      logcObs[i] ~ normal(
        log(cHat[i]), 
        (b1 * measure[i] * log(cHat[i])) + ((b2 * (1 - measure[i]) * log(cHat[i]))) );
    }
}
...
generated quantities{
  ...
  for(i in 1:nObs){
    cHatPred[i] = twoCPTM_model_simple(
      ...
      cObsCond[i] = exp(normal_rng(
        log(cHat[i]), 
        abs((b1 * measure[i] * log(cHat[i])) + ((b2 * (1 - measure[i]) * log(cHat[i]))))
        )); // Predicciones individuales
      
      cObsPred[i] = exp(normal_rng(
        log(cHatPred[i]), 
        abs((b1 * measure[i] * log(cHatPred[i])) + (b2 * (1 - measure[i]) * log(cHatPred[i])))
        )); // Predicciones poblacionales
        //print(measure[i]);
        //print((b1 * measure[i] * log(cHatPred[i])) + (b2 * (1 - measure[i]) * log(cHatPred[i])));
  }
}
