...
generated quantities{
  ...
  for(i in 1:nObs){
    cHatPred[i] = twoCPTM_model_simple(
      ...
      cObsCond[i] = exp(normal_rng(log(cHat[i]), abs(b*log(cHat[i])))); // Predicciones individuales
      cObsPred[i] = exp(normal_rng(log(cHatPred[i]), abs(b*log(cHatPred[i])))); // Predicciones poblacionales
  }
}
