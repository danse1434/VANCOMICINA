...
G1 <- df[df$DD %in% seq(1500, 3000, by = 500), ] %>% 
  ...
  facet_grid(DD ~ II, 
             labeller = labeller(
               .cols = function(x) paste0("II: ", x, "h"), 
               DD = function(x) paste0("DD: ", as.numeric(x)/1e3, "g"))) + 
  scale_color_manual(values = c('red', 'green3', 'blue'), name = 'Tinf (h)') + 
  scale_fill_manual(values = c('red', 'green3', 'blue'), name = 'Tinf (h)') + 
  labs(title = 'Simulación Vancomicina SS', 
       subtitle = bquote('CLCR: sigue distribución; CLCR ~ '~italic(LN)*'(4.78, 0.023)'))

G1

ggsave('050_perfilPlasmaticoCrCldesconocido.pdf', G1, 'pdf', 'figures', 1, 8, 8 * 0.75)
...
G2 <- df1[df1$DD %in% seq(1500, 3000, by = 500), ] %>% 
  ...
  facet_grid(DD ~ II, 
             labeller = labeller(
               .cols = function(x) paste0("II: ", x, "h"), 
               .rows = function(x) paste0("DD: ", as.numeric(x)/1e3, "g"))) + 
  scale_color_manual(values = c('red', 'green3', 'blue'), name = 'Tinf (h)') + 
  scale_fill_manual(values = c('red', 'green3', 'blue'), name = 'Tinf (h)')
...
G3 <- df1[df1$DD %in% seq(1500, 3000, by = 500), ] %>% 
  ...
  ylab('Densidad') +
  facet_grid(DD ~ II, labeller = labeller(
    .cols = function(x) paste0("II: ", x, "h"), 
    .rows = function(x) paste0("DD: ", as.numeric(x)/1e3, "g"))) + 
  scale_color_manual(values = c('red', 'green3', 'blue'), name = 'Tinf (h)') + 
  scale_fill_manual(values = c('red', 'green3', 'blue'), name = 'Tinf (h)')
