...
# 
project.file <-
  file.path('M2CPTM_nobs_2_aditv', 'M2CPTM_nobs_2_aditv.mlxtran')
...
res1 <- simulx(project = project.file, 
               group = list(size = N), 
               settings = list(load.design=TRUE, digits = 3L))
...
data_TAD <-
  read_csv(file.path('M2CPTM_nobs_2_aditv', 'data', "data_TAD.csv"), na = '.')
