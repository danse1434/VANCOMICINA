...
# Compilar archivo C++
sourceCpp(file.path(auxdir, 'src', '013_funciones_analisis_boot.cpp'))
...
for (i in 1:1000) {
  param_list[[i]] = read_csv(
    glue('{bootdir}/B{i}/M2CPTM_nobs_2_aditv/populationParameters.txt'),
                             col_types = cols())
}
...
ggsave(glue('{auxdir}/figures/001_resultados_bootstrap.pdf'), G1, 'pdf',
       width = 9, height = 8)
...
# Almacenamiento del gráfico
ggsave(glue('{auxdir}/figures/002_originales_bootstrap.pdf'), G2, 'pdf',
       width = 8, height = 6)
...
# Almacenamiento del gráfico
ggsave(glue('{auxdir}/figures/003_bootstrap_dots.pdf'), G3, 'pdf',
       width = 6, height = 4)
...
param_table_gt <- param_table %>%
  ...
  gt::tab_header(gt::md('**Resultados bootstrap no parámétrico**'),
                 gt::md('**Modelo base para VAN por dos métodos - error aditivo**')) %>% 
...
gt::gtsave(param_table_gt, '004_resultados_bootstrap.html', 
...
for (i in 1:1000) {
  convergence_list[[i]] =
    read_csv(
      glue(
        '{bootdir}/B{i}/M2CPTM_nobs_2_aditv/ChartsData/Saem/CvParam.txt'
      ),
      col_types = cols()
    )
}
...
# Almacenamiento del gráfico
ggsave(glue('{auxdir}/figures/005_trayectorias_Iter.pdf'), GITER, 'pdf',
       width = 8, height = 6)
...
ggsave(glue('{auxdir}/figures/006_filtro_convergencia.pdf'), G_conv_filt, 'pdf', 
         width = 7, height = 5)
...
# Almacenamiento del gráfico
ggsave(glue('{auxdir}/figures/008_resultados_bootstrap_limpio.pdf'), G1b, 'pdf',
       width = 8, height = 6)
...
# Almacenamiento del gráfico
ggsave(glue('{auxdir}/figures/009_bootstrap_dots_limpia.pdf'), G3b, 'pdf',
       width = 6, height = 4)
...
# Almacenamiento de tabla de datos de HTML
gt::gtsave(param_df3_gt, '010_IC_bootstrap.html', 
