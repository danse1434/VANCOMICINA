...
# Compilar archivo C++
sourceCpp(file.path(auxdir, 'src', '14_funciones_analisis_boot.cpp'))
...
for (i in 1:1000) {
  param_list[[i]] = read_csv(
    glue('{bootdir}/B{i}/M2CPTM_nobs_2_comb1/populationParameters.txt'),
                             col_types = cols())
}
...
ggsave(glue('{auxdir}/figures/1_resultados_bootstrap.pdf'), G1, 'pdf',
       width = 9, height = 8)
...
# Almacenamiento del gráfico
ggsave(glue('{auxdir}/figures/2_originales_bootstrap.pdf'), G2, 'pdf',
       width = 8, height = 6)
...
# Almacenamiento del gráfico
ggsave(glue('{auxdir}/figures/3_bootstrap_dots.pdf'), G3, 'pdf',
       width = 6, height = 4)
...
param_table_gt <- param_table %>%
  ...
  gt::tab_header(gt::md('**Resultados bootstrap no parámétrico**'),
                 gt::md('**Modelo base para VAN determinada por quimioluminiscencia - error proporcional**')) %>% 
...
for (i in 1:1000) {
  convergence_list[[i]] =
    read_csv(
      glue(
        '{bootdir}/B{i}/M2CPTM_nobs_2_comb1/ChartsData/Saem/CvParam.txt'
      ),
      col_types = cols()
    )
}
...
# Almacenamiento del gráfico
ggsave(glue('{auxdir}/figures/5_trayectorias_Iter.pdf'), GITER, 'pdf',
       width = 8, height = 6)
...
ggsave(glue('{auxdir}/figures/6_filtro_convergencia.pdf'), G_conv_filt, 'pdf', 
         width = 7, height = 5)
...
# Almacenamiento del gráfico
ggsave(glue('{auxdir}/figures/8_resultados_bootstrap_limpio.pdf'), G1b, 'pdf',
       width = 8, height = 6)
...
# Almacenamiento del gráfico
ggsave(glue('{auxdir}/figures/9_bootstrap_dots_limpia.pdf'), G3b, 'pdf',
       width = 6, height = 4)
...
param_df3_gt <- param_df3 %>%
  ...
  ungroup() %>% 
    mutate(parameter = 
             fct_recode(parameter,'V1'='V1_pop','V2'='V2_pop', 'Cl'='Cl_pop', 
                        'Q'='Q_pop', 'f' = 'f_pop')) %>%
  ...
  gt::tab_spanner('Resultados Bootstrap (n = 998)', 
  ...
  gt::tab_header(gt::md('**Resultados bootstrap no parámétrico**'),
                 gt::md('**Modelo base VAN con determinaciones por quimioluminiscencia**')) %>% 
  gt::tab_options(table.font.size = "smaller",
                  data_row.padding = gt::px(3)) %>% 
  gt::tab_footnote('Obtenido mediante bootstrap no paramétrico', 
                   gt::cells_column_spanners(spanners = "Resultados Bootstrap (n = 998)")) %>% 
  gt::tab_footnote('Obtenido mediante matriz de info. de Fisher (FIM)', 
