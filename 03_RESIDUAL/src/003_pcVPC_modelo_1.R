...
source(file.path('src', '010_fun_pcVPC.R'), encoding = 'UTF-8')
...
data_pred_1 <- data_pred %>%
  .[, gr := ntile(time, 100)] %>%
  .[, .(time_bin = mean(time),
        ME_bin = quantile(x = y2, probs = 0.50)), by = .(gr)]
...
dfr_percs %>%
  .[, gr := ntile(time, 100), by = .(design)] %>%
  .[, `:=`(time = round(time, 3), y2 = round(y2, 3))] %>%
  .[data_pred, on = c('id', 'gr', 'time'), `:=`(PRED = i.y2, PRED_bin = i.ME_bin)] %>% 
  .[, pc_y2 := y2 * PRED_bin/PRED]
...
dfr_percs_1 <- dfr_percs %>% 
  .[, .(
    TIME = mean(time),
    ME   = quantile(x = pc_y2, probs = 0.50),
    LI   = quantile(x = pc_y2, probs = 0.10),
    LS   = quantile(x = pc_y2, probs = 0.90)
), by = .(design, gr)]
...
setDT(data_OBS_PRED)
setnames(data_OBS_PRED, old='y2', new='PRED')
...
data_OBS_PRED %>% 
  .[data_OBS_1, on = c('id', 'time'), y2 := i.y2] %>% 
  .[, pc_y2 := y2 * PRED_bin/PRED]
...
ggsave(file.path(auxdir, 'figures', '11_pcVPC_percentil.pdf'), g_percs, 'pdf',
       width = 6, height = 5)

# Almacenar RDS
saveRDS(g_percs, file.path(auxdir, 'figures', 'RDS', '11_pcVPC_percentil.rds'))
