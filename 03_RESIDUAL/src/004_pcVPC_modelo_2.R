...
##  simulación por medio del paquete MLXR de Monolix (modelo Combinado). 
##  
...
#-------------------------------------------------------------------------------#
modelName <- 'M2CPTM_nobs_2_comb1'
auxdir <- file.path('.', modelName)
project.file <- file.path(auxdir, paste0(modelName, '.mlxtran'))
paramet.file <- file.path(auxdir, modelName)
...
g_percs <-
  ...
  facet_wrap(. ~ YTYPE, labeller = labeller(.rows = label_both)) +
