...
##  
## Propósito del Script: Análisis de distribución de residuales para varios 
## modelos de error (modelos con observaciones por microbiología y 
## quimioluminiscencia)
##  
...
ggsave(file.path('figures', '001_res_TAD_quimiol.pdf'), G_RES_T_Comp, 'pdf', 
       width = 8, height = 8)
...
ggsave(file.path('figures', '002_res_PRE_quimiol.pdf'), G_RES_C_Comp, 'pdf', 
       width = 8, height = 8)
...
gt_res_norm <- res_norm %>% 
  ...
  tab_header(title = md('**Batería de Pruebas de Normalidad - Modelo de Residuales**'),
             subtitle = md('**Modelos con determinaciones por quimioluminiscencia**')) %>% 
  cols_label(
...
ggsave('004_res_qqplot_quimiol.pdf', plot_resid_comp, 'pdf', 
       'figures', 1, 8, 6, 'in')
...
plot_hist_comp <- res_norm %>% 
  ...
  geom_label(
    ...
    )), y = 0.65, x = 3.0, hjust=0.5, vjust=1, size=2, colour='black' ) +
  theme(
    legend.position = 'none',
    panel.grid.major = element_line(color = 'gray85'),
    panel.grid.minor = element_line(color = 'gray93')
...
ggsave('005_res_histogramas_quimiol.pdf', plot_hist_comp, 'pdf', 
       'figures', 1, 8, 6, 'in')
...
ggsave('006_res_ecdf_quimiol.pdf', plot_acum_comp, 'pdf', 
       'figures', 1, 8, 6, 'in')