...
dfr_percs <- data_list %>%
  ...
  mutate(
    ...
    ME   = map_dbl(data, ~ quantile(.x$y1, probs = 0.50)),
    LI   = map_dbl(data, ~ quantile(.x$y1, probs = 0.10)),
    LS   = map_dbl(data, ~ quantile(.x$y1, probs = 0.90))
  )
#-------------------------------------------------------------------------------#
...
ggsave(file.path(auxdir, 'figures', 'M2a_VPC_percentil.pdf'), g_percs, 'pdf', 
       width = 6, height = 5)
...
data_pred_1 <- data_pred %>%
  ...
  summarise(time_bin = mean(time),
            ME_bin = quantile(x = y1, probs = 0.50))
...
dfr_percs_pcVPC <- data_ls_df %>%
  mutate(pcy1 = y1.x * ME_bin / y1.y) %>%
  group_by(simId, gr) %>%
  summarise(
    TIME = mean(time),
    ME   = quantile(x = pcy1, probs = 0.50),
    LI   = quantile(x = pcy1, probs = 0.10),
    LS   = quantile(x = pcy1, probs = 0.90)
  )
#-------------------------------------------------------------------------------#
# Calcular intervalos de confianza del 95% para cada intervalo de predicción
# Se debe realizar un resumen del valor de IP dentro de cada bin, entre todas 
...
#-------------------------------------------------------------------------------#
# Almacenar RDS
saveRDS(g_percs,  file.path(auxdir, 'figures', 'RDS', 'M2a_VPC_percentil.rds'))
saveRDS(g_percs1, file.path(auxdir, 'figures', 'RDS', 'M2a_pcVPC_percentil.rds'))
