...
# Script de funciones
source(file.path('src', '011_funciones.R'), encoding = 'UTF-8')
# Abrir datos de correlación
proy_dir <- file.path('M2CPTM_nobs_2_aditv')
...
simulatedParameterDF <-
  expand.grid(simulatedParameter, simulatedParameter)

# simulatedParameterDF <- simulatedParameterDF %>% 
#   mutate(plot = map2(Var1, Var2, ~corr_plot(data_corr_1, .x, .y)))   
# simulatedParameterDF$plot[[1]]
...
correl_mat_com <-
  (
    hist_plot(data_corr_1, eta_Cl_simulated, expression(eta ~ (Cl))) +
      ...
      corr_plot(data_corr_1, eta_Cl_simulated, eta_V1_simulated) +
      hist_plot(data_corr_1, eta_V1_simulated, expression(eta ~ (V[1]))) +
      corr_plot(data_corr_1, eta_Q_simulated, eta_V1_simulated) +
      corr_plot(data_corr_1, eta_V2_simulated, eta_V1_simulated) +
      corr_plot(data_corr_1, eta_Cl_simulated, eta_Q_simulated) +
      corr_plot(data_corr_1, eta_V1_simulated, eta_Q_simulated) +
      ...
      corr_plot(data_corr_1, eta_V1_simulated, eta_V2_simulated) +
...
ggsave(file.path('figures', '001_graficoCorrelacion1.png'), correl_mat_com, device = 'png', 
       width = 6 * 1.5, height = 4 * 1.7, dpi = 720)

ggsave(file.path('figures', '001_graficoCorrelacion2.pdf'), correl_mat_com, device = 'pdf', 
...
data_esteta_2 <- data_esteta_1 %>%
  ...
  mutate(
    ...
    parameter = factor(parameter, levels = c('Eta_Cl', 'Eta_V1', 'Eta_Q', 'Eta_V2'))
  ) 
...
ggsave(G_boxplot, filename = file.path('figures', '002_boxplotEtaEst1.png'), 
        device = 'png', width = 5, height = 4, dpi = 720)
ggsave(G_boxplot, filename = file.path('figures', '002_boxplotEtaEst1.pdf'), 
        device = 'pdf', width = 5, height = 4)

#-------------------------------------------------------------------------------#
# Correlación entre Etas -----------------------------------------------------
#-------------------------------------------------------------------------------#
replicate_average <- function(x, y) {
  mean(map2_dbl(x, y, ~ prod(.x, .y)))
}

rep_correlation <- data_corr_1 %>%
  select(matches('^eta|ID')) %>%
  group_by(ID) %>% 
  nest() %>% 
  mutate(corr = map(data, ~colpair_map(.x, replicate_average)))
  
id_correlation <- rep_correlation$corr %>% 
  map_dfr( ~.x, .id='id') %>% 
  pivot_longer(cols = matches('^eta')) %>% 
  group_by(term, name) %>% 
  summarise(
    tval = mean(value)/(sd(value)/sqrt(n())),
    p = 2 * pt(-abs(tval), n()-1)
    )

id_corr_gt <- id_correlation %>% 
  ungroup() %>% 
  gt(groupname_col = 'term', ) %>% 
  gt::tab_header(md('**Correlaciones entre parámetros**')) %>% 
...
gt::gtsave(id_corr_gt, filename = '004_estadisticoTval.html', 
           path = glue::glue('{getwd()}/figures'))

# pivot_wider(id_correlation, 
#             id_cols=term, names_from = name, values_from=tval) %>% 
#   add_column(Parametro = 'Estadístico T') %>% 
#   bind_rows(pivot_wider(id_correlation, 
#                         id_cols=term, names_from = name, values_from=p) %>% 
#               add_column(Parametro = 'Valor p')) %>%
#   ungroup() %>% 
#   gt()
...
gt_FishMatrix <- data_FishMatrix %>% 
  ...
  gt::fmt_number(columns = 2:dim(data_FishMatrix)[2],
                 decimals = 3) %>% 
  gt::tab_header(title = md('**Matriz de correlación - Modelo Base Vancomicina**')) %>% 
  ...
  formatoCondicionalCovarianza('omega_V1') %>% 
  formatoCondicionalCovarianza('omega_Q') %>% 
  formatoCondicionalCovarianza('omega_V2') %>%
  formatoCondicionalCovarianza('a1') %>%
  formatoCondicionalCovarianza('a2')

gt::gtsave(gt_FishMatrix, filename = '003_matrizCorrelacion.html', 
           path = glue::glue('{getwd()}/figures'))
