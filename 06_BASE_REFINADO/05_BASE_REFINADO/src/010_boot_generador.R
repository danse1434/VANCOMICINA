...
project.file <- file.path(auxdir, 'BASE.mlxtran')
paramet.file <- file.path(auxdir, 'BASE')
...
data <- read_csv(file.path('data', 'data_TAD.csv'),
                 na = '.')
...
for (i in 1:1000) {
  file.copy(file.path(glue('BASE.mlxproperties')),
            file.path(bootdir, glue('B{i}/')))
}

#-------------------------------------------------------------------------------#
# Apertura del archivo de control
fileName <- file.path('BASE.mlxtran')
...
A1 <-
  ...
  str_replace_all('(?<=a2\\s\\=\\s\\{value\\=)\\d\\.\\d+(?=\\,)','1.0') %>%
  ...
  str_replace("exportpath = 'M2CPTM_nobs_2_aditv_corr2'", "exportpath = 'BASE'") %>% 
...
for (i in 1:1000) {
  ...
  write_lines(B,
              file.path(bootdir, glue('B{i}/BASE.mlxtran')), sep = '\n')
}
