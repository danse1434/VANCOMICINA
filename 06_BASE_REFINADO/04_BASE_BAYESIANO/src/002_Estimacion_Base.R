##------------------------------------------------------------------------------#
## Nombre del Script: Estimación bayesiana con distribuciones a priori difusas --
##  
## Propósito del Script: con este script se realiza una estimación bayesiana para 
## definir el modelo base definitivo. 
##  
...
if(!file.exists(file.path('models', paste0(modelName, "Fit.Rsave")))){
  ...
  test <- stan(
    file   = file.path('src', paste0(modelName, '.stan')), # Modelo Stan
    data   = stan_d, # Datos
    chains = 1,      # Cadenas
