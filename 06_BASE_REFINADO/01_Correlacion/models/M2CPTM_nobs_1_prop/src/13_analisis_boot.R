...
vectorParametros <- c('V1_pop', 'V2_pop', 'Cl_pop', 'Q_pop','beta_Cl_tSCRMGDL',
...
for (i in 1:1000) {
  param_list[[i]] = read_csv(
    glue('{bootdir}/B{i}/M2CPTM_nobs_1_prop/populationParameters.txt'),
                             col_types = cols())
}
...
for (i in 1:1000) {
  convergence_list[[i]] =
    read_csv(
      glue(
        '{bootdir}/B{i}/M2CPTM_nobs_1_prop/ChartsData/Saem/CvParam.txt'
      ),
      col_types = cols()
    )
}
